...
public class MenuServiceImpl implements MenuService {
    ...
    public List<Map<String, Object>> getMenuByGroup(String groupId) {
        String sql = " select menu.menu_id ,menu.name,menu.parent_id, " +
                "   case when (select count(1) from tb_neopermission_group_menu where group_id = '"+groupId+"' and menu_id = menu.menu_id and del_flag='0') <>0 then 1 else 0 end checked" +
                "   from tb_neopermission_menu menu where del_flag = '0'";
        return getJt().queryForList(sql);
    }
    ...
    public List<Map<String, Object>> findAllMenu() {
        String sql = "select menu_id, name, parent_id,sort," +
                "   IFNULL((select max(sort) from tb_neopermission_menu where parent_id = menu.menu_id),0) as next_sort" +
                "   from tb_neopermission_menu menu  where del_flag = '0' order by parent_id asc , sort asc";
        return getJt().queryForList(sql);
    }
    ...
    public Integer getNextSort(String menuId) {
        String sql = "select IFNULL(max(sort),0) sort from tb_neopermission_menu where parent_id = '"+menuId+"'";
        Integer sort = Integer.parseInt(getJt().queryForMap(sql).get("sort").toString());
        if(sort != 0){
            int multiple = 1;
            while(sort % 10 == 0){
                sort = sort /10;
            ...
            sql = "select IFNULL(max(sort),0) as sort from tb_neopermission_menu where menu_id = '"+menuId+"'";
            sort = Integer.parseInt(getJt().queryForMap(sql).get("sort").toString());
            int multiple = 1;
            while(sort % 10 == 0){
                sort = sort /10;
                multiple = multiple * 10;
    ...
    public List<Menu> getMenuExpectButtonByParentId(String menuId, String userId) {
        String sql = "select DISTINCT menu.* from tb_neopermission_menu menu \n" +
                "   join tb_neopermission_role_menu role_menu on menu.menu_id = role_menu.menu_id\n" +
                "   join tb_neopermission_role role on role_menu.role_id = role.role_id\n" +
                "   join tb_neopermission_user_role user_role on role.role_id = user_role.role_id" +
