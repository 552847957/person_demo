...
public interface AssessmentRepository extends JpaRepository<Assessment,String> {
    ...
    @Query("select a from Assessment a where a.uid=?1 and a.createDate<?2  and a.delFlag=0")
    Page<Assessment> getAssessmentHistory(String uid, Date flag, Pageable pageable);

    @Query(nativeQuery = true ,value = "select * from app_tb_patient_assessment  where uid=?1  and del_flag=0" +
