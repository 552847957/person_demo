...
public interface ArticleFavoriteRepository extends JpaRepository<ArticleFavorite,String> {
    @Query(nativeQuery = true,value = "select * from app_tb_neoarticle_favorite where user_id = ?1")
    List<ArticleFavorite> queryByUid(String uid);
    @Query(nativeQuery = true,value = "select * from app_tb_neoarticle_favorite where user_id = ?1 and article_id=?2")
