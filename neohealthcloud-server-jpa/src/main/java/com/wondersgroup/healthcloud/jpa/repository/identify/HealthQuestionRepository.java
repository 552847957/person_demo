...
public interface HealthQuestionRepository extends JpaRepository<HealthQuestion, String> {
	...
	@Query(value="select count(hq) from HealthQuestion hq where registerid=?1 and type = 1 and result is not null  and result != '' and delFlag = '0'")
	Integer getTotalQuestion(String registerid);

	@Query(value="select hq from HealthQuestion hq where registerid=?1 and type = 1 and result is not null and result != '' and delFlag = '0'")
	...
	@Query(nativeQuery = true,value="select * from app_tb_healthquestion  " +
			" where registerid=?1 and type = 1 and del_flag = '0' and result is not null  and result != '' order by testtime desc limit 1")
