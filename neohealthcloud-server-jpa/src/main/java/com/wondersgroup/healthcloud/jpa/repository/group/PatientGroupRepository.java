...
public interface PatientGroupRepository extends JpaRepository<PatientGroup, Integer> {
    ...
    @Query(nativeQuery=true,value="SELECT * FROM app_tb_patient_group p WHERE p.doctor_id=?1 AND p.name=?2 AND p.del_flag='0' ")
    PatientGroup findIsNameRepeated(String doctorId,String name);
