...
public class ManageOrderDTO {
    private static final Logger log = Logger.getLogger(ManageHospitalDTO.class);
    ...
    /**
     * 上下午标志 1:上午 2:下午 3:晚上
     */
    @JsonProperty("time_range")
    private String timeRange;


    public ManageOrderDTO(OrderDto order) {
        if(order!=null){
            this.id = order.getId();
            ...
            this.patientIdcard = order.getUserCardId();//患者身份证

            this.patientMobile = order.getUserPhone();//患者手机号

            this.mediCardId = order.getMediCardId();//社保卡

            this.orderTime = DateFormatter.dateTimeFormat(order.getCreateDate());
            this.userMobile = order.getRegmobilephone();
            this.timeRange = order.getTimeRange();
